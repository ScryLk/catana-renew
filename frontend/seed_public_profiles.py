...
# IMPORTANTE: Ajuste os imports conforme seus models
...
class Command(BaseCommand):
    ...
    def _create_organization_and_sede(self):
        ...
        # from your_app.models import Organization, Sede
        ...
        # MOCK - Remova isso quando tiver os models
        class MockOrg:
            id = 1
            name = 'Catana Demo'
    ...
    def _create_catalogs(self, users_profiles, catalogs_per_user, org, sede):
        ...
        for idx, (user, profile) in enumerate(users_profiles):
            ...
            for _ in range(num_catalogs):
                ...
                #     sede=sede,
                #     created_by=user,
                #     is_public=random.random() > 0.2,  # 80% público
                ...
                class MockCatalog:
                    pass
                catalog = MockCatalog()
                catalog.id = len(catalogs) + 1
                catalog.title = title
                catalog.created_by = user
                catalog.is_public = random.random() > 0.2
    ...
    def _create_products(self, catalogs, categories, products_per_catalog, org, sede):
        ...
        for cat_idx, catalog in enumerate(catalogs):
            ...
            for i in range(num_products):
                ...
                num_gallery = random.randint(1, 4)
                image_gallery = '|'.join(random.sample(image_urls, num_gallery))
